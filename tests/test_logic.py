...
def test_sugestao_so_para_quem_deve():
    assert logic.sugestao_pagamento(0, "Ana") is None
    assert logic.sugestao_pagamento(100, "Ana") is None


def test_sugestao_sem_responsavel_e_nula():
    assert logic.sugestao_pagamento(-300, None) is None


def test_sugestao_e_pagar_a_caixa_tudo_o_que_se_deve():
    assert logic.sugestao_pagamento(-250, "Ana") == {"para_caixa": True, "nome": "Caixa (Ana)", "valor_cent": 250}
